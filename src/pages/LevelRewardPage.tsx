...
const LevelRewardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        ...
        res.forEach(r => { map[r.item] = r.avgPrice; });
        setPriceMap(map);
      } catch (e) {
  ...
  const handleCalculate = () => {
    ...
    if (compareLevel) {
      const compareLevelNum = parseInt(compareLevel);
      if (!isNaN(compareLevelNum)) {
        setCalculatedCompareLevel(compareLevelNum);
        setSelectedCompareRaids(getAvailableRaids(compareLevelNum).slice(0, 3).map(r => r.name));
      }
    } else {
  ...
  const calculateLevelReward = (level: number, isCompare: boolean = false): LevelReward => {
    ...
    const targetRaids = isCompare 
      ? availableRaids.filter(raid => selectedCompareRaids.includes(raid.name))
      : availableRaids.filter(raid => selectedRaids.includes(raid.name));
        ...
        <Container maxWidth={false} sx={{ py: 12, maxWidth: '850px !important', height: 'auto' }}>
          {currentTab === 0 && (
            <Box sx={{ mt: 5 }}>
              ...
              />
              {hasCalculated && calculatedMainLevel !== null && mainReward && (
                <>
          ...
          {currentTab === 0 ? (
            <>
              {hasCalculated && mainReward && (mainReward.totalTradableGold > 0 || mainReward.totalBoundGold > 0) && (
                <RewardCard
                  title="기준 캐릭터"
                  tradableGold={mainReward.totalTradableGold}
                  boundGold={mainReward.totalBoundGold}
              ...
              {hasCalculated && mainReward && (mainReward.raidTradableGold > 0 || mainReward.raidBoundGold > 0) && (
                <RewardCard
                  title="레이드 보상"
                  tradableGold={mainReward.raidTradableGold}
                  boundGold={mainReward.raidBoundGold}
              ...
              {hasCalculated && mainReward && (mainReward.chaosTradableGold > 0 || mainReward.chaosBoundGold > 0) && (
                <RewardCard
                  title="카오스 던전 보상"
                  tradableGold={mainReward.chaosTradableGold}
                  boundGold={mainReward.chaosBoundGold}
